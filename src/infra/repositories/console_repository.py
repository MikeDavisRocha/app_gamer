...
class ConsoleRepository(IConsoleRepository):
    ...
    async def create(self, console: Console) -> Console:
        db_console = ConsoleModel(name=console.name, company=console.company)
    ...
    async def list_all(self) -> List[Console]:
        ...
        return [
            Console(id=m.id, name=m.name, company=m.company, created_at=m.created_at, updated_at=m.updated_at)
            for m in models
        ]
